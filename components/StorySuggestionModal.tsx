      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl flex flex-col max-h-[90vh]">
        <div className="p-4 border-b border-gray-200 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-gray-800">AI Story Suggestion</h3>
              <p className="text-sm text-gray-500">Provide a simple idea for the AI to flesh out. You can also reference previous pages to continue the story.</p>
            </div>
          </div>
        ...
        <div className="p-4 flex-grow overflow-y-auto grid grid-cols-2 gap-4">
          ...
          <div className="flex flex-col gap-4 pr-4 border-r border-gray-200">
            <div>
              <label htmlFor="story-idea" className="block text-sm font-semibold text-gray-600 mb-1">Story Idea</label>
              <textarea
                id="story-idea"
                value={premise}
                onChange={(e) => setPremise(e.target.value)}
                placeholder="e.g., Kaito confronts Anya on the rooftop"
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <label htmlFor="continue-story" className="text-sm font-medium text-gray-700">Reference previous pages</label>
            </div>
            <button
            ...
            >
              {isLoading ? 'Suggesting...' : 'Generate Suggestion'}
            </button>
          </div>

          {/* Right: Output */}
          <div className="flex flex-col">
            <h4 className="text-sm font-semibold text-gray-600 mb-2">AI Suggestion</h4>
              ...
              {isLoading ? (
                ...
                <div className="space-y-4 text-sm">
                  {suggestion.panels.map((panel) => (
                    <div key={panel.panel}>
                      <p className="font-bold text-gray-800">Panel {panel.panel}</p>
                      <p className="text-gray-600 pl-2 border-l-2 border-gray-200 ml-1 mt-1">
                        <strong className="font-semibold">Description:</strong> {panel.description}
                      </p>
                      {panel.dialogue && (
                        <p className="text-gray-600 pl-2 border-l-2 border-gray-200 ml-1 mt-1">
                          <strong className="font-semibold">Dialogue:</strong> <em>{panel.dialogue}</em>
                        </p>
                      )}
                    </div>
                  ))}
                ...
                <p className="text-gray-400 text-center text-xs py-10">AI Suggestion will appear here.</p>
              )}
            </div>
          </div>
        </div>
