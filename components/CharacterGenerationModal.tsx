...
export function CharacterGenerationModal({ onClose, onSave, characters }: CharacterGenerationModalProps) {
  ...
  // We don't have access to pages here, so we pass an empty array.
  // The character limit check is what's important.
  const subscription = useSubscription([], characters);
  ...
  const handleAddDraft = () => {
    // Check against the total number of existing characters plus current drafts
    if (characters.length + drafts.length >= subscription.limits.characters) {
        alert(`You have reached your character limit of ${subscription.limits.characters} for the ${subscription.planName} plan. Please upgrade for more.`);
        return;
    }
  ...
  const handleGenerate = async (id: number) => {
    ...
    if (!draft || !draft.name) {
      handleUpdateDraft(id, { error: "Enter character name" });
      return;
    }
    handleUpdateDraft(id, { isGenerating: true, error: null, generatedSheet: null });
    
    try {
      let result: { data: string, usage: Omit<any, 'id' | 'timestamp'> };
      if (draft.creationMode === 'new') {
        if (draft.referenceImages.length === 0) throw new Error("Please provide a name and a reference image.");
        result = await generateCharacterSheet(draft.referenceImages, draft.name, draft.sheetColorMode);
      } else { // fromReference
        if (draft.referenceCharacterIds.length === 0) throw new Error("Please select an existing character to reference.");
        if (!draft.concept) throw new Error("Please enter a concept for the new character.");
      ...
      addUsageRecord(result.usage);
      handleUpdateDraft(id, { generatedSheet: result.data, isGenerating: false });
    } catch (e) {
  ...
  const handleEdit = async (id: number) => {
    ...
    if (!draft || !draft.generatedSheet || !draft.editPrompt) {
      handleUpdateDraft(id, { error: "Please provide an edit description." });
      return;
    }
    handleUpdateDraft(id, { isEditing: true, error: null });

    try {
        const { data: result, usage } = await editCharacterSheet(draft.generatedSheet, draft.name, draft.editPrompt);
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col">
        <div className="p-6 border-b border-gray-200 flex justify-between items-center">
          <div>
            <h2 className="text-xl font-bold text-gray-800">Create Characters</h2>
            <p className="text-sm text-gray-500">Add multiple characters and generate their sheets at once to streamline your workflow.</p>
          </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                {drafts.map(draft => {
                                ...
                                {draft.generatedSheet ? (
                                    <div className="flex flex-col gap-4">
                                        <div className="relative aspect-[4/3] bg-gray-100 rounded-md flex items-center justify-center">
                                           <img src={draft.generatedSheet} alt="Generated Sheet" className="max-h-full w-auto object-contain rounded-md" />
                                        ...
                                        <div className="flex items-center gap-2">
                                            <button onClick={() => handleGenerate(draft.id)} className="flex-1 flex items-center justify-center gap-1.5 bg-gray-200 text-gray-700 font-semibold py-1.5 px-3 rounded-md hover:bg-gray-300 text-xs"><RedoAltIcon className="w-4 h-4" /> Regenerate</button>
                                            <button onClick={() => handleSave(draft.id)} className="flex-1 flex items-center justify-center gap-1.5 bg-green-600 text-white font-semibold py-1.5 px-3 rounded-md hover:bg-green-500 text-xs"><CheckCircleIcon className="w-4 h-4" /> Save Character</button>
                                        </div>
                                        <div>
                                            <textarea
                                                value={draft.editPrompt}
                                                onChange={(e) => handleUpdateDraft(draft.id, { editPrompt: e.target.value })}
                                                placeholder="e.g., 'add glasses', 'make the hair curly'"
                                                className="w-full bg-gray-50 border border-gray-300 rounded-md p-2 text-xs h-16 resize-none"
                                            />
                                            <button onClick={() => handleEdit(draft.id)} disabled={draft.isEditing || !draft.editPrompt} className="w-full mt-1.5 flex items-center justify-center gap-1.5 bg-gray-700 text-white font-semibold py-1.5 px-3 rounded-md hover:bg-gray-600 disabled:bg-gray-400 text-xs"><SparklesIcon className="w-4 h-4"/> Update Sheet</button>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="flex flex-col gap-3">
                                        <div className="flex rounded-lg bg-gray-100 p-1 w-full text-xs font-semibold">
                                            <button onClick={() => handleUpdateDraft(draft.id, { creationMode: 'new' })} className={`w-1/2 p-1.5 rounded-md ${draft.creationMode === 'new' ? 'bg-white shadow' : 'text-gray-500'}`}>Create New</button>
                                            <button onClick={() => handleUpdateDraft(draft.id, { creationMode: 'fromReference' })} disabled={characters.length === 0} className={`w-1/2 p-1.5 rounded-md ${draft.creationMode === 'fromReference' ? 'bg-white shadow' : 'text-gray-500'} disabled:text-gray-400 disabled:cursor-not-allowed`}>From Reference</button>
                                        </div>
                                        
                                        {draft.creationMode === 'new' ? (
                                            <>
                                                <textarea value={draft.description} onChange={(e) => handleUpdateDraft(draft.id, { description: e.target.value })} placeholder="e.g., A kind-hearted but clumsy hero, who is secretly a villain." className="w-full bg-gray-50 border border-gray-300 rounded-md p-2 text-xs h-16 resize-y" />
                                            ...
                                            <div className="flex flex-col gap-2">
                                                 <div className="bg-gray-100 border border-gray-200 rounded-lg p-2">
                                                    <p className="text-xs font-semibold text-gray-600 mb-2">Select Reference Character</p>
                                                ...
                                                <textarea value={draft.concept} onChange={(e) => handleUpdateDraft(draft.id, { concept: e.target.value })} placeholder="e.g., A stoic knight with a large scar over his eye and silver armor" className="w-full bg-gray-50 border border-gray-300 rounded-md p-2 text-xs h-16 resize-y" />
                                            </div>
                                        )}

                                        <div className="flex rounded-lg bg-gray-100 p-1 w-full mt-auto">
                                            <button onClick={() => handleUpdateDraft(draft.id, { sheetColorMode: 'monochrome' })} className={`w-1/2 px-3 py-1.5 text-xs font-semibold rounded-md transition-colors ${draft.sheetColorMode === 'monochrome' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:bg-gray-200'}`}> Monochrome </button>
                                            <button onClick={() => handleUpdateDraft(draft.id, { sheetColorMode: 'color' })} className={`w-1/2 px-3 py-1.5 text-xs font-semibold rounded-md transition-colors ${draft.sheetColorMode === 'color' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:bg-gray-200'}`}> Color </button>
                                        </div>
                                        <button onClick={() => handleGenerate(draft.id)} disabled={draft.isGenerating || !draft.name || (draft.creationMode === 'new' && draft.referenceImages.length === 0) || (draft.creationMode === 'fromReference' && (!draft.referenceCharacterIds.length || !draft.concept)) } className="w-full bg-indigo-600 text-white font-bold py-2 rounded-lg hover:bg-indigo-500 disabled:bg-gray-400">
                                            {draft.isGenerating ? "Generating..." : "Generate Sheet"}
                                        </button>
                                    </div>
                                )}
                 ...
                 <div onClick={handleAddDraft} className="bg-gray-100 rounded-lg border-2 border-dashed border-gray-300 hover:border-indigo-500 hover:bg-white transition-colors flex flex-col items-center justify-center text-gray-500 cursor-pointer min-h-[300px]">
                    ...
                    <p className="mt-2 font-semibold">Add Character</p>
                 </div>
            </div>
        ...
        <div className="p-4 bg-white border-t border-gray-200 flex justify-between items-center">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={handleSaveAllAndClose} className="bg-green-600 text-white font-bold py-2 px-5 rounded-lg hover:bg-green-500 transition-colors text-sm">
                {hasUnsavedDrafts ? "Save & Close" : "Close"}
            </button>
          </div>
        </div>
      </div>
