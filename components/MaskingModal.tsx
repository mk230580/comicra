    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex flex-col items-center justify-center z-50 p-4" onClick={onClose}>
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[95vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="p-4 border-b border-gray-200 flex justify-between items-center">
                <div>
                    <h3 className="text-lg font-bold text-gray-800">Mask Area to Edit</h3>
                    <p className="text-sm text-gray-500">Use the brush to paint over the areas of the image you want to change. The edit prompt will only apply to the painted regions.</p>
                </div>
            ...
            <div className="p-4 flex-grow flex items-center justify-center bg-gray-100 relative min-h-0">
                <div className="absolute top-2 left-2 z-10 bg-white shadow-lg rounded-full border border-gray-200 p-2 flex items-center gap-4">
                    ...
                    <button onClick={handleUndo} className="p-2 hover:bg-gray-200 rounded-full" title="Undo" disabled={history.length <= 1}><UndoIcon className="w-5 h-5 text-gray-700" /></button>
                    <button onClick={handleClear} className="p-2 hover:bg-red-100 rounded-full" title="Clear Canvas"><TrashIcon className="w-5 h-5 text-red-500" /></button>
                </div>
            ...
            <div className="p-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3">
                ...
                <button onClick={handleSave} className="bg-indigo-600 text-white font-bold py-2 px-5 rounded-lg hover:bg-indigo-500 transition-colors text-sm">Save Mask</button>
            </div>
        </div>
    </div>
  );
