...
>(({ shapes, onShapesChange, characters, aspectRatio, viewTransform, onViewTransformChange, isDraggingCharacter, onUndo, onRedo, canUndo, canRedo, proposalImage, proposalOpacity, isProposalVisible, onProposalSettingsChange, onApplyLayout, isFullscreen, onToggleFullscreen }, ref) => {
  ...
  useEffect(() => {
      const handleKeyDown = (e: KeyboardEvent) => {
          ...
          if (e.key === ' ' && !isSpacePressed.current && !isTyping) {
  ...
  }, [selectedShapeId, onUndo, onRedo, canUndo, canRedo, shapes, onShapesChange]);

  const getMousePos = useCallback((e: React.MouseEvent | MouseEvent | TouchEvent | React.DragEvent) => {
    const svg = svgRef.current;
  ...
  const handleMouseDown = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    switch(activeTool) {
        ...
        case 'arrow': {
            ...
            const arrowShape: ArrowShape = { id, type: 'arrow', points: [startPoint, startPoint], x: pos.x, y: pos.y, strokeColor: '#FF0000', strokeWidth: brushSize };
            newShape = arrowShape;
            setAction({type: 'creating', shape: newShape});
            break;
        }
        ...
        case 'creating': {
            ...
            if (action.shape.type === 'arrow') {
                newShapes = shapes.map(s => {
                    ...
                    if (s.type === 'arrow') {
                        const newPoints: [{x:number, y:number}, {x:number, y:number}] = [s.points[0], pos];
  ...
  const savePose = (characterId: string, pose: Pose) => {
    onShapesChange(shapes.map(s => {
      if (s.id === characterId && s.type === 'image') {
        ...
        if (pose?.type === 'skeleton') {
            const modalCharBox = { x: 0, y: 0, w: 200, h: 400 }; 
            const newSkeletonData: SkeletonData = {};
            for (const key in pose.data) {
    ...
    <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm h-full flex flex-col relative" onDragOver={handleDragOver} onDrop={handleDrop}>
        ...
        {isDraggingCharacter && (
            <div className="absolute inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 pointer-events-none rounded-xl">
                <p className="text-white text-2xl font-bold bg-indigo-600/80 px-6 py-3 rounded-lg">Drop Character onto a Panel</p>
            </div>
        )}
        ...
        <div className="flex justify-between items-center mb-4 px-2">
            <h2 className="text-lg font-semibold text-gray-700">1. Create Visual Layout</h2>
            <button onClick={onToggleFullscreen} className="p-2 rounded-full hover:bg-gray-200" onMouseEnter={(e) => handleTooltipShow(e, isFullscreen ? 'Exit Fullscreen' : 'Enter Fullscreen')} onMouseLeave={handleTooltipHide}>
      ...
      <div className="flex-grow w-full h-full flex items-start justify-center gap-4 bg-gray-100 rounded-lg overflow-hidden p-4 relative">
        <div className="bg-white shadow-lg rounded-full border border-gray-200 p-2 flex flex-col items-center gap-1 z-10">
            <button onClick={onUndo} disabled={!canUndo} className="p-3 rounded-full hover:bg-gray-200 text-gray-700 disabled:text-gray-300 disabled:cursor-not-allowed" onMouseEnter={(e) => handleTooltipShow(e, 'Undo (Ctrl+Z)')} onMouseLeave={handleTooltipHide}><UndoIcon className="w-5 h-5"/></button>
            <button onClick={onRedo} disabled={!canRedo} className="p-3 rounded-full hover:bg-gray-200 text-gray-700 disabled:text-gray-300 disabled:cursor-not-allowed" onMouseEnter={(e) => handleTooltipShow(e, 'Redo (Ctrl+Y)')} onMouseLeave={handleTooltipHide}><RedoIcon className="w-5 h-5"/></button>
            <div className="w-8 h-px bg-gray-300 my-1"></div>
            <button onClick={() => { setActiveTool('select'); }} className={`p-3 rounded-full ${activeTool === 'select' ? 'bg-indigo-600 text-white' : 'hover:bg-gray-200 text-gray-700'}`} onMouseEnter={(e) => handleTooltipShow(e, 'Select & Move (V)')} onMouseLeave={handleTooltipHide}><SelectIcon className="w-5 h-5"/></button>
            <button onClick={() => setActiveTool('pan')} className={`p-3 rounded-full ${activeTool === 'pan' ? 'bg-indigo-600 text-white' : 'hover:bg-gray-200 text-gray-700'}`} onMouseEnter={(e) => handleTooltipShow(e, 'Pan Canvas (H or Spacebar)')} onMouseLeave={handleTooltipHide}><HandIcon className="w-5 h-5"/></button>
            <div className="w-8 h-px bg-gray-300 my-1"></div>
            <button onClick={() => setActiveTool('panel')} className={`p-3 rounded-full ${activeTool === 'panel' ? 'bg-indigo-600 text-white' : 'hover:bg-gray-200 text-gray-700'}`} onMouseEnter={(e) => handleTooltipShow(e, 'Draw Panel (P)')} onMouseLeave={handleTooltipHide}><PolygonIcon className="w-5 h-5"/></button>
            <button onClick={() => setActiveTool('text')} className={`p-3 rounded-full ${activeTool === 'text' ? 'bg-indigo-600 text-white' : 'hover:bg-gray-200 text-gray-700'}`} onMouseEnter={(e) => handleTooltipShow(e, 'Add Text (T)')} onMouseLeave={handleTooltipHide}><TextToolIcon className="w-5 h-5"/></button>
            <button onClick={() => setActiveTool('draw')} className={`p-3 rounded-full ${activeTool === 'draw' ? 'bg-indigo-600 text-white' : 'hover:bg-gray-200 text-gray-700'}`} onMouseEnter={(e) => handleTooltipShow(e, 'Draw Freehand (B)')} onMouseLeave={handleTooltipHide}><BrushIcon className="w-5 h-5"/></button>
            <button onClick={() => setActiveTool('arrow')} className={`p-3 rounded-full ${activeTool === 'arrow' ? 'bg-indigo-600 text-white' : 'hover:bg-gray-200 text-gray-700'}`} onMouseEnter={(e) => handleTooltipShow(e, 'Draw Arrow (A)')} onMouseLeave={handleTooltipHide}><ArrowIcon className="w-5 h-5"/></button>
            <button onClick={() => imageUploadRef.current?.click()} className="p-3 rounded-full hover:bg-gray-200 text-gray-700" onMouseEnter={(e) => handleTooltipShow(e, 'Upload Image')} onMouseLeave={handleTooltipHide}><UploadIcon className="w-5 h-5"/></button>
            
            <div className="w-8 h-px bg-gray-300 my-1"></div>
            
            <div className="flex flex-col items-center gap-1 bg-gray-100 rounded-full p-1">
                <button onClick={() => {setActiveTool('bubble'); setActiveBubbleType('rounded')}} className={`p-2 rounded-full ${activeTool === 'bubble' && activeBubbleType === 'rounded' ? 'bg-indigo-500 text-white' : 'hover:bg-gray-200 text-gray-700'}`} onMouseEnter={(e) => handleTooltipShow(e, 'Rounded Bubble')} onMouseLeave={handleTooltipHide}><BubbleToolIcon className="w-5 h-5"/></button>
                <button onClick={() => {setActiveTool('bubble'); setActiveBubbleType('oval')}} className={`p-2 rounded-full ${activeTool === 'bubble' && activeBubbleType === 'oval' ? 'bg-indigo-500 text-white' : 'hover:bg-gray-200 text-gray-700'}`} onMouseEnter={(e) => handleTooltipShow(e, 'Oval Bubble')} onMouseLeave={handleTooltipHide}><CircleIcon className="w-5 h-5"/></button>
                <button onClick={() => {setActiveTool('bubble'); setActiveBubbleType('rect')}} className={`p-2 rounded-full ${activeTool === 'bubble' && activeBubbleType === 'rect' ? 'bg-indigo-500 text-white' : 'hover:bg-gray-200 text-gray-700'}`} onMouseEnter={(e) => handleTooltipShow(e, 'Rectangular Bubble')} onMouseLeave={handleTooltipHide}><SquareIcon className="w-5 h-5"/></button>
            </div>

            <div className="w-8 h-px bg-gray-300 my-1"></div>
            <button onClick={clearCanvas} className="p-3 rounded-full hover:bg-red-500 hover:text-white text-gray-700" onMouseEnter={(e) => handleTooltipShow(e, 'Clear Canvas')} onMouseLeave={handleTooltipHide}><TrashIcon className="w-5 h-5"/></button>
        </div>
        ...
        {(activeTool === 'draw' || activeTool === 'arrow') && (
            <div className="absolute top-6 left-24 z-10 bg-white shadow-lg rounded-lg border border-gray-200 p-2 flex items-center gap-3 animate-fade-in">
                <label htmlFor="brush-color" className="text-sm font-medium text-gray-700">Color:</label>
                ...
                <label htmlFor="brush-size" className="text-sm font-medium text-gray-700 ml-2">Size:</label>
        ...
        <div className="w-full h-full relative" onWheel={handleWheel}>
            ...
            >
                ...
                {sortedShapes.map((shape) => {
                    ...
                    return (
                        ...
                        >
                                ...
                                <>
                                    ...
                                    {isSelected && (
                                        <>
                                            ...
                                            <foreignObject x={bbox.x - 30/viewTransform.scale} y={bbox.y} width={25/viewTransform.scale} height={bbox.height} >
                                                 <div className="flex flex-col h-full justify-around items-center">
                                                   <button onMouseDown={(e) => e.stopPropagation()} onClick={() => shape.tail ? removeBubbleTail(shape.id) : addBubbleTail(shape.id)} className="p-0.5 bg-white rounded-full shadow border hover:bg-gray-100 flex items-center justify-center">
                                                       {shape.tail ? <XIcon className="w-4 h-4 text-red-500" /> : <PlusIcon className="w-4 h-4" />}
                                                   </button>
                                                </div>
                                            </foreignObject>
                                        </>
                            ...
                            {shape.type === 'drawing' && (
                                <path d={getDrawingPath(shape)} fill="none" stroke={isSelected ? "rgba(128, 90, 213, 1)" : shape.strokeColor} strokeWidth={(isSelected ? shape.strokeWidth + 2 : shape.strokeWidth) / viewTransform.scale} strokeLinecap="round" strokeLinejoin="round" />
                            )}
                            {shape.type === 'arrow' && (
                                <>
                                    <line
                                        ...
                                        strokeWidth={shape.strokeWidth / viewTransform.scale}
                                        markerEnd={isSelected ? "url(#arrowhead-selected)" : "url(#arrowhead)"}
                                    />
                                    {isSelected && (
                                        <>
                                            <circle cx={shape.points[0].x} cy={shape.points[0].y} r={6/viewTransform.scale} fill="white" stroke="rgba(128, 90, 213, 1)" strokeWidth={2/viewTransform.scale} cursor="move" onMouseDown={(e) => handleShapeInteraction(e, shape, 'arrowHandle', 0)} />
                                            <circle cx={shape.points[1].x} cy={shape.points[1].y} r={6/viewTransform.scale} fill="white" stroke="rgba(128, 90, 213, 1)" strokeWidth={2/viewTransform.scale} cursor="move" onMouseDown={(e) => handleShapeInteraction(e, shape, 'arrowHandle', 1)} />
                                        </>
                                    )}
                                </>
                            )}
                            {shape.type === 'image' && (
                                <>
                                    <image href={shape.href} x={shape.x} y={shape.y} width={shape.width} height={shape.height} />
                                    {renderPose(shape)}
                                    {isSelected && (
                                        <>
                                            ...
                                            <foreignObject x={bbox.x + bbox.width} y={bbox.y} width={30/viewTransform.scale} height={30/viewTransform.scale}>
                                                <button onMouseDown={(e) => e.stopPropagation()} onClick={() => openPoseEditor(shape)} className="p-1 bg-white rounded-full shadow border hover:bg-gray-100 flex items-center justify-center">
                                                    <EditPoseIcon className="w-5 h-5" />
                                                </button>
                                            </foreignObject>
                                        </>
                                    )}
                ...
                {cursorPreview && (
                    <circle cx={cursorPreview.x} cy={cursorPreview.y} r={brushSize / (2 * viewTransform.scale)} fill="none" stroke="rgba(0,0,0,0.5)" strokeWidth={1/viewTransform.scale} pointerEvents="none" />
                )}
                </g>
            </svg>
            <div className="absolute bottom-4 right-4 z-10 flex flex-col gap-2 bg-white shadow-lg rounded-full border border-gray-200 p-2">
                <button onClick={zoomIn} className="p-2 hover:bg-gray-200 rounded-full" onMouseEnter={(e) => handleTooltipShow(e, 'Zoom In')} onMouseLeave={handleTooltipHide}><PlusIcon className="w-5 h-5"/></button>
                <button onClick={fitAndCenterCanvas} className="p-2 hover:bg-gray-200 rounded-full" onMouseEnter={(e) => handleTooltipShow(e, 'Fit to Screen')} onMouseLeave={handleTooltipHide}><span className="text-xs font-bold">{Math.round(viewTransform.scale * 100)}%</span></button>
                <button onClick={zoomOut} className="p-2 hover:bg-gray-200 rounded-full" onMouseEnter={(e) => handleTooltipShow(e, 'Zoom Out')} onMouseLeave={handleTooltipHide}><MinusIcon className="w-5 h-5"/></button>
            </div>
             {proposalImage && (
                <div className="absolute bottom-4 left-4 z-10 bg-white shadow-lg rounded-full border border-gray-200 p-2 flex items-center gap-4">
                    <button onClick={() => onProposalSettingsChange({ isProposalVisible: !isProposalVisible })} className="p-2 hover:bg-gray-200 rounded-full" onMouseEnter={(e) => handleTooltipShow(e, isProposalVisible ? 'Hide Proposal' : 'Show Proposal')} onMouseLeave={handleTooltipHide}>
                        {isProposalVisible ? <EyeIcon className="w-5 h-5"/> : <EyeOffIcon className="w-5 h-5"/>}
                    </button>
                    <input type="range" min="0" max="1" step="0.05" value={proposalOpacity} onChange={(e) => onProposalSettingsChange({ proposalOpacity: Number(e.target.value) })} className="w-24" />
                    <button onClick={onApplyLayout} className="py-1 px-3 bg-indigo-600 text-white font-semibold rounded-full text-sm hover:bg-indigo-500" onMouseEnter={(e) => handleTooltipShow(e, 'Use this layout as the page image')} onMouseLeave={handleTooltipHide}>
                        Apply
                    </button>
                </div>
            )}
