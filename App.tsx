...
import type { Character, Page, CanvasShape, ViewTransform, StorySuggestion, PanelShape, ImageShape, AnalysisResult, ArrowShape } from './types';
import { AddUserIcon, TrashIcon, LinkIcon } from './components/icons';
import { useAuth } from './contexts/AuthContext';
import { Auth } from './components/Auth';
import { fetchUserData, saveUserData } from './services/apiService';
...
export default function App(): React.ReactElement {
  ...
  const subscription = useSubscription(pages, characters);
            ...
            try {
                ...
                if (userData.pages && userData.pages.length > 0) {
                    setPages(userData.pages);
                     if (userData.currentPageId && userData.pages.some(p => p.id === userData.currentPageId)) {
                        setCurrentPageId(userData.currentPageId);
                    } else {
                        setCurrentPageId(userData.pages[0].id);
                    }
                } else {
                    const newPage = { ...initialPage, id: Date.now().toString(), name: 'Page 1' };
                    setPages([newPage]);
                    setCurrentPageId(newPage.id);
                }

                setCharacters(userData.characters || []);
                setWorldview(userData.worldview || '');
            } catch (e) {
                console.error("Failed to load user data", e);
                setError("Could not load your comic data. Please try refreshing.");
                const newPage = { ...initialPage, id: Date.now().toString(), name: 'Page 1' };
                setPages([newPage]);
                setCurrentPageId(newPage.id);
            } finally {
  ...
  useEffect(() => {
    if (isAuthenticated && user?.id && !isDataLoading) {
  ...
  }, [pages, characters, worldview, currentPageId, isAuthenticated, user?.id, isDataLoading]);
  ...
  const handleGenerateImage = useCallback(async () => {
    ...
    try {
        ...
        const { data: result, usage } = await generateMangaPage(relevantCharacters, panelLayoutImage, currentPage.sceneDescription, colorMode, previousPageData, generateEmptyBubbles);
  ...
  }, [currentPage, pages, currentPageId, characters, colorMode, handleUpdateCurrentPage, viewMode, generateEmptyBubbles, addUsageRecord]);
  ...
  const handleColorize = useCallback(async () => {
      ...
      try {
          ...
          const { data: coloredImage, usage } = await colorizeMangaPage(currentPage.generatedImage, relevantCharacters);
          addUsageRecord(usage);
  ...
  }, [currentPage, characters, handleUpdateCurrentPage, addUsageRecord]);
 ...
 const handleEditImage = useCallback(async (editPrompt: string, editReferenceImages: string[] | null) => {
    ...
    setError(null);
    try {
        const { data: editedImage, usage } = await editMangaPage(currentPage.generatedImage, editPrompt, currentMask || undefined, editReferenceImages || undefined);
        addUsageRecord(usage);
  ...
  }, [currentPage?.generatedImage, currentMask, handleUpdateCurrentPage, addUsageRecord]);
  ...
  const handleGenerateDetailedStory = async (premise: string, shouldContinue: boolean) => {
      ...
      try {
          ...
          const { data: suggestion, usage } = await generateDetailedStorySuggestion(premise, worldview, characters, previousPagesContext);
          addUsageRecord(usage);
    ...
    const handleGenerateLayoutProposal = async () => {
        ...
        try {
            const { data: { proposalImage }, usage } = await generateLayoutProposal(
  ...
  const handleStartAutoGeneration = async (numPages: number, startFromPage: number = 1) => {
      ...
      if (characters.length === 0 && worldview === '') {
          setError("At least one character must be registered or a worldview must be entered to start auto-generation.");
          return;
      }
      
      stopAutoGenerationRef.current = false;
      setAssistantModeState({ isActive: true, totalPages: numPages, currentPageNumber: startFromPage, statusMessage: "Starting continuous auto-generation...", hasError: false });
      ...
      if (startFromPage > 1) {
        ...
        localCurrentPageId = currentLocalPages[retryPageIndex]?.id || currentLocalPages[currentLocalPages.length - 1].id;
          ...
          if (lastPage.assistantProposalImage || lastPage.shapes.length > 0 || lastPage.sceneDescription) {
              ...
              const newPage: Page = { ...initialPage, id: newPageId, name: `Page ${currentLocalPages.length + 1}` };
      ...
      try {
        for (let i = startFromPage; i <= numPages; i++) {
          if (stopAutoGenerationRef.current) {
            setAssistantModeState(prevState => ({ ...prevState!, statusMessage: "Stopping..." }));
            break;
          }
          const pageIndex = currentLocalPages.findIndex(p => p.id === localCurrentPageId);
          let pageObject = currentLocalPages[pageIndex];

          setAssistantModeState({ isActive: true, totalPages: numPages, currentPageNumber: i, statusMessage: `Suggesting story for page ${i}/${numPages}...` });
          ...
          const { data: story, usage: storyUsage } = await generateDetailedStorySuggestion(storyPremise, worldview, characters, prevPageContext ? [prevPageContext] : undefined);
          addUsageRecord(storyUsage);
          ...
          setAssistantModeState({ isActive: true, totalPages: numPages, currentPageNumber: i, statusMessage: `Suggesting layout for page ${i}/${numPages}...` });
          const { data: { proposalImage }, usage: layoutUsage } = await generateLayoutProposal(sceneDescription, characters, pageObject.aspectRatio, previousPageLayout);
          addUsageRecord(layoutUsage);
          ...
          if (i < numPages) {
              ...
              const newPage: Page = { ...initialPage, id: nextPageId, name: `Page ${currentLocalPages.length + 1}`, aspectRatio: pageObject.aspectRatio };
      ...
      setAssistantModeState(prevState => prevState ? {...prevState, statusMessage: 'Auto-generation Complete!', isActive: false} : null);
  ...
  const handleDeletePage = (idToDelete: string) => {
      ...
      const newPages = pages.filter(p => p.id !== idToDelete);
      setPages(newPages);
  ...
  const handleAnalyzeResult = useCallback(async () => {
    if (!currentPage?.panelLayoutImage || !currentPage?.generatedImage) {
        setError("Cannot analyze without both a layout and a generated image.");
        return;
    }
    setIsAnalyzing(true);
    setAnalysisResult(null);
    ...
    try {
        ...
        const { data: result, usage } = await analyzeAndSuggestCorrections(
  ...
  }, [currentPage, characters, addUsageRecord]);
  ...
  const handleVideoProducerClick = () => {
    if (subscription.canUseVideoProducer) {
        setCurrentView('video-producer');
    } else {
        setShowUpgradeModal(true);
    }
  };
  ...
  if (isDataLoading) {
      return (
          <div className="flex items-center justify-center h-screen bg-gray-100 text-gray-700">
              <svg className="animate-spin h-8 w-8 text-indigo-600 mr-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
              </svg>
              Loading your Comicra...
          </div>
      );
  }
  ...
  if (!currentPage) {
    ...
    return (
        <div className="flex flex-col items-center justify-center h-screen bg-gray-100">
            <p className="text-lg mb-4">No pages found for this user.</p>
            <button onClick={handleCreateFirstPage} className="bg-indigo-600 text-white font-bold py-2 px-5 rounded-lg hover:bg-indigo-500">
  ...
  return (
    <div className="flex flex-col h-screen font-sans bg-gray-50 text-gray-800">
      <Header 
        ...
        onVideoProducerClick={handleVideoProducerClick}
      ...
      {showUpgradeModal && <UpgradeModal onClose={() => setShowUpgradeModal(false)} onUpgrade={() => { setShowUpgradeModal(false); setShowPricingModal(true); }} />}
        ...
        {currentView === 'video-producer' ? (
          ...
          <>
            <div ref={editorAreaRef} className="flex flex-1 bg-gray-50">
              <aside className={`w-64 bg-white p-4 border-r border-gray-200 flex-col gap-4 flex-shrink-0 transition-all duration-300 ease-in-out ${isSidebarOpen ? 'flex' : 'hidden'}`}>
                <div>
                  <div className="flex justify-between items-center mb-2">
                    <h3 className="font-bold text-sm text-gray-500 tracking-wider uppercase">Pages</h3>
                    <span className="text-xs font-semibold text-gray-400">{subscription.pageUsage}</span>
                  </div>
                  <div className="mb-4 relative">
                      <label htmlFor="aspect-ratio-select" className="block text-xs font-medium text-gray-500 mb-1">Aspect Ratio</label>
                      <button
                  ...
                  {assistantModeState?.isActive ? (
                    ...
                    <button onClick={() => handleAddPage()} disabled={!subscription.canCreatePage} className="border border-dashed border-gray-300 bg-gray-50 rounded-lg p-2 text-center font-semibold text-gray-500 mt-2 cursor-pointer hover:border-indigo-500 hover:text-indigo-700 disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed disabled:hover:border-gray-300">
                      + Add Page
                    </button>
                    </>
                  )}
                ...
                <div className="flex flex-col gap-2">
                    <div className="flex justify-between items-center mb-2">
                        <h3 className="font-bold text-sm text-gray-500 tracking-wider uppercase">Characters</h3>
                        <span className="text-xs font-semibold text-gray-400">{subscription.characterUsage}</span>
                    </div>
                  <div className="flex flex-col gap-2">
                      ...
                      {characters.map(char => (
                          <div key={char.id} className="flex items-center justify-between gap-3 p-2 rounded-md bg-gray-100 border border-gray-200 group relative">
                              <div className="absolute -top-2 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-xs px-2 py-0.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">Drag me!</div>
                              <div className="flex items-center gap-3 flex-grow cursor-grab" draggable onDragStart={(e) => { e.dataTransfer.setData('text/plain', char.id); e.dataTransfer.effectAllowed = 'copy'; setIsDraggingCharacter(true); }} onDragEnd={() => setIsDraggingCharacter(false)}>
                  ...
                  <button onClick={() => setShowCharacterModal(true)} className="w-full mt-2 flex items-center justify-center gap-2 border border-dashed border-gray-300 bg-gray-50 rounded-lg p-2 text-center font-semibold text-gray-500 cursor-pointer hover:border-indigo-500 hover:text-indigo-700">
                    <AddUserIcon className="w-5 h-5" /> Add Character
                  </button>
                </div>
              </aside>
            ...
            <aside className="w-96 bg-white p-6 border-l border-gray-200 flex flex-col gap-6 overflow-y-auto flex-shrink-0">
              {viewMode === 'result' && currentPage.generatedImage && !error ? (
                ...
                <div className="flex flex-col gap-6 h-full">
                  {!anyLoading && <h2 className="text-xl font-bold text-gray-800">Generate Your Comicra</h2>}
                  {anyLoading ? (
                            ...
                            <>
                                ...
                                <p className="text-gray-500 mt-4 font-semibold">
                                   {assistantModeState?.isActive ? assistantModeState.statusMessage :
                                    ...
                                    isSuggestingStory ? "Suggesting..." : 
                                    isSuggestingLayout ? "Suggesting..." : 
                                    isColoring ? "Colorizing page..." : "Generating..."}
                                </p>
                    ...
                    <>
                      <GenerationControls
                        ...
                        canCreatePage={subscription.canCreatePage}
