  ...
  if (isLoading || isColoring || isAnalyzing) {
    return (
        <div className="bg-white rounded-xl h-full flex flex-col items-center justify-center p-6 text-center">
            ...
            <p className="text-lg font-semibold mt-4">{isAnalyzing ? "Analyzing..." : (isLoading && !isColoring ? "Editing..." : '')}</p>
            {isColoring ? <p className="text-gray-500 mt-1">Colorizing page...</p> : isAnalyzing ? null : <LoadingMessage />}
        </div>
    );
  }
        ...
        {error ? (
            ...
            <>
                <div className="relative group">
                     <img src={generatedContent.image} alt="Generated Comicra page" className="w-full object-contain rounded-md shadow-lg border border-gray-200" />
                     {mask && (
                ...
                <div className="flex flex-col gap-4 p-2">
                    <div className="grid grid-cols-2 gap-2">
                        {isMonochromeResult && (
                         ...
                         <button onClick={onRegenerate} className="w-full bg-gray-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-gray-500 transition-colors flex items-center justify-center gap-2 text-sm">
                            <RedoAltIcon className="w-4 h-4" /> Regenerate
                        </button>
                        <button onClick={onStartMasking} className="w-full bg-purple-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-purple-500 transition-colors flex items-center justify-center gap-2 text-sm">
                           <BrushIcon className="w-4 h-4" /> Edit with Mask
                        </button>
                    </div>
                    ...
                    <button 
                        ...
                        <WandIcon className="w-4 h-4" /> Analyze & Suggest Corrections
                    </button>

                    {analysisResult && (
                        <div className="border-t border-gray-200 pt-4 animate-fade-in">
                            <div className="flex justify-between items-center mb-2">
                                <h3 className="text-md font-semibold text-gray-700">AI Analysis Report</h3>
                            ...
                            <div className="bg-gray-100 p-3 rounded-md border border-gray-200 text-sm">
                                ...
                                {analysisResult.has_discrepancies ? (
                                    ...
                                    >
                                        <SparklesIcon className="w-5 h-5" /> Apply Correction
                                    </button>
                                ) : (
                    ...
                    <div className="border-t border-gray-200 pt-4">
                        <h3 className="text-md font-semibold text-gray-700 mb-2">Edit Result</h3>
                        {mask && (
                            <div className="mb-2 flex justify-between items-center bg-indigo-50 p-2 rounded-md border border-indigo-200">
                                ...
                                <button onClick={onClearMask} className="text-xs text-indigo-500 hover:underline font-bold">Clear Mask</button>
                            </div>
                        )}
                        <textarea
                            value={editPrompt}
                            onChange={(e) => setEditPrompt(e.target.value)}
                            placeholder="e.g., 'Erase text in all speech bubbles'"
                        ...
                        />
                         <div className="mt-2">
                             <h4 className="text-xs font-semibold text-gray-500 mb-1 uppercase tracking-wider">Upload Reference</h4>
                         ...
                         <div className="mt-2">
                            <h4 className="text-xs font-semibold text-gray-500 mb-1 uppercase tracking-wider">Characters</h4>
                            {characters.length > 0 ? (
                                <div className="grid grid-cols-4 gap-2">
                                    {characters.map(char => (
                                ...
                                <p className="text-xs text-gray-400 text-center bg-gray-100 p-2 rounded-md">Create a character to get started.</p>
                            )}
                         </div>
                        ...
                        >
                            <SparklesIcon className="w-5 h-5" /> Apply Edits
                        </button>
                    </div>
                </div>
            </>
